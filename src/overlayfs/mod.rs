...
impl RealInode {
    ...
    pub(crate) fn stat64(&self, ctx: &Context) -> Result<stat64> {
    ...
    pub(crate) fn readdir(&self, ctx: &Context) -> Result<HashMap<String, RealInode>> {
    ...
    pub(crate) fn create_whiteout(&self, ctx: &Context, name: &str) -> Result<RealInode> {
    ...
    pub(crate) fn link(&self, ctx: &Context, ino: u64, name: &str) -> Result<RealInode> {
        if !self.in_upper_layer {
            return Err(Error::from_raw_os_error(libc::EROFS));
        }
...
impl OverlayInode {
    ...
    pub fn new_from_real_inode(name: &str, ino: u64, real_inode: RealInode) -> Self {
    ...
    pub fn new_from_real_inodes(name: &str, ino: u64, real_inodes: Vec<RealInode>) -> Result<Self> {
        if real_inodes.len() == 0 {
        ...
        for ri in real_inodes {
            ...
            if first {
                first = false;
                new = Self::new_from_real_inode(name, ino, ri);
    ...
    pub fn scan_childrens(self: &Arc<Self>, ctx: &Context) -> Result<Vec<OverlayInode>> {
        ...
        for (name, real_inodes) in all_layer_inodes {
            ...
            let mut new = Self::new_from_real_inodes(name.as_str(), 0, real_inodes)?;
            new.path = format!("{}/{}", self.path, name);
            childrens.push(new);
        }
    ...
    pub(crate) fn add_upper_inode(self: &Arc<Self>, ri: RealInode, clear_lowers: bool) {
...
impl OverlayFs {
    ...
    pub fn load_directory(&self, ctx: &Context, node: &Arc<OverlayInode>) -> Result<()> {
        ...
        let mut all_inodes = self.inodes.lock().unwrap();
        ...
        for mut child in childrens.into_iter() {
            ...
            // FIXME: inode can be reclaimed, we need a better inode allocator. @weizhang555
            ...
            all_inodes.insert(ino, arc_child.clone());
        }
        ...
        match self.lookup_node_ignore_enoent(ctx, parent_node.inode, name)? {
            Some(n) => {
                ...
                pnode.handle_upper_inode_locked(&mut |parent_real_inode| -> Result<bool> {
                    ...
                    n.add_upper_inode(child_dir, true);
                    Ok(false)
                })?;
            }
            None => {
                ...
                pnode.handle_upper_inode_locked(&mut |parent_real_inode| -> Result<bool> {
                    ...
                    let ino = self.alloc_inode()?;
                    let child_dir = parent_real_inode.mkdir(ctx, name, mode, umask)?;
                    let ovi = OverlayInode::new_from_real_inode(name, ino, child_dir);
        ...
        match self.lookup_node_ignore_enoent(ctx, parent_node.inode, name)? {
            ...
            None => {
                ...
                pnode.handle_upper_inode_locked(&mut |parent_real_inode| -> Result<bool> {
                    ...
                    let ino = self.alloc_inode()?;
                    let child_ri = parent_real_inode.mknod(ctx, name, mode, rdev, umask)?;
                    let ovi = OverlayInode::new_from_real_inode(name, ino, child_ri);
    ...
    pub fn do_link(
        ...
        match self.lookup_node_ignore_enoent(ctx, new_parent.inode, name)? {
            Some(n) => {
                ...
                // n is definitely a whiteout now.
    ...
    pub fn create_upper_dir(
        ...
        if !pnode.in_upper_layer() {
            self.create_upper_dir(ctx, &pnode, None)?; // recursive call
        }
        let mut child = None;
            ...
            match parent_upper_inode {
                Some(parent_ri) => {
                    let ri = match mode_umask {
                        Some((mode, umask)) => {
                            parent_ri.mkdir(ctx, node.name.as_str(), mode, umask)?
                        }
                        None => parent_ri.mkdir(ctx, node.name.as_str(), st.st_mode, 0)?,
                    };
    ...
    pub fn copy_symlink_up(
        ...
        // // Why lookup again? @weizhang555
        // let parent_node = self.lookup_node(ctx, parent_node.inode, "")?;

        // // TODO: why lookup again? @weizhang555
        ...
        if !parent_node.in_upper_layer() {
            self.create_upper_dir(ctx, &parent_node, None)?;
        }
    ...
    pub fn copy_regfile_up(
        ...
        parent_node.handle_upper_inode_locked(&mut |parent_upper_inode| -> Result<bool> {
            ...
            let (inode, _) = parent_real_inode.create(ctx, node.name.as_str(), args)?;
    ...
    pub fn copy_node_up(
        ...
        // directory
        if utils::is_dir(st) {
            self.create_upper_dir(ctx, &node, None)?;
